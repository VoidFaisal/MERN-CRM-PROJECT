...
const updateUser = async (req, res) => {
  ...
  if (role == "admin" && id) {
    const user = req.body;
    try {
      const Management = await management.findByIdAndUpdate(id, user, {
        new: true,
        runValidators: true,
      });
...
const createCustomer = async (req, res) => {
  ...
  if (role == "sales representative") {
    try {
      ...
      const { name, email, phone_number, company, address, industry, notes } =
        req.body;
...
const filterCustomer = async (req, res) => {
  ...
  try {
    const Customer = await customer.find(queryObject);
    if (!Customer) {
      res.status(404).json({ msg: "Data Does Not Exists " });
    }
    res.status(200).json({ Customer });
